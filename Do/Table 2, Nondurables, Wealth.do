...
This tables describes nondurable spending done in wealth tertiles. 
...
xtile wealthquart = h_atotb, nquantiles(3)
...
preserve 
	collapse (mean) nondurables, by(time wealthquart)
	reshape wide nondurables, i(time) j(wealthquart)
...
preserve 
	collapse (median) nondurables, by(time wealthquart)
	reshape wide nondurables, i(time) j(wealthquart)
...
preserve 
	collapse (p10) difnondurables, by(time wealthquart)
	reshape wide difnondurables, i(time) j(wealthquart)
...
preserve 
	collapse (p25) difnondurables, by(time wealthquart)
	reshape wide difnondurables, i(time) j(wealthquart)
...
preserve 
	collapse (median) difnondurables, by(time wealthquart)
	reshape wide difnondurables, i(time) j(wealthquart)
...
preserve 
	collapse (p75) difnondurables, by(time wealthquart)
	reshape wide difnondurables, i(time) j(wealthquart)
...
preserve 
	collapse (p90) difnondurables, by(time wealthquart)
	reshape wide difnondurables, i(time) j(wealthquart)
...
//create wealth quartiles
xtile wealthquart = h_atotb, nquantiles(4)
//Median Percent Change
gen difnondurables = (nondurables - L.nondurables) / L.nondurables
...
drop combine _merge
rename time Wealth_Quartiles
rename nondurables1 First
rename nondurables2 Second
rename nondurables3 Third
rename nondurables All

replace First = round(First) if Wealth_Quartiles == "Pre-retirement" | Wealth_Quartiles == "Post-retirement"
replace Second = round(Second) if Wealth_Quartiles == "Pre-retirement" | Wealth_Quartiles == "Post-retirement"
replace Third = round(Third) if Wealth_Quartiles == "Pre-retirement" | Wealth_Quartiles == "Post-retirement"
replace All = round(All) if Wealth_Quartiles == "Pre-retirement" | Wealth_Quartiles == "Post-retirement"
replace First = round(First, .1) if Wealth_Quartiles != "Pre-retirement" | Wealth_Quartiles != "Post-retirement"
replace Second = round(Second, .1) if Wealth_Quartiles != "Pre-retirement" | Wealth_Quartiles != "Post-retirement"
replace Third = round(Third, .1) if Wealth_Quartiles != "Pre-retirement" | Wealth_Quartiles != "Post-retirement"
replace All = round(All, .1) if Wealth_Quartiles != "Pre-retirement" | Wealth_Quartiles != "Post-retirement"

tostring First Second Third All, replace force format(%9.1f)
replace First = substr(First, 1, 2) + "," + substr(First, 3, 3) if Wealth_Quartiles == "Pre-retirement" | Wealth_Quartiles == "Post-retirement"
replace Second = substr(Second, 1, 2) + "," + substr(Second, 3, 3) if Wealth_Quartiles == "Pre-retirement" | Wealth_Quartiles == "Post-retirement"
replace Third = substr(Third, 1, 2) + "," + substr(Third, 3, 3) if Wealth_Quartiles == "Pre-retirement" | Wealth_Quartiles == "Post-retirement"
replace All = substr(All, 1, 2) + "," + substr(All, 3, 3) if Wealth_Quartiles == "Pre-retirement" | Wealth_Quartiles == "Post-retirement"

replace First = "" if Wealth_Quartiles == "Means:" | Wealth_Quartiles == "Medians:"
replace Second = "" if Wealth_Quartiles == "Means:" | Wealth_Quartiles == "Medians:"
replace Third = "" if Wealth_Quartiles == "Means:" | Wealth_Quartiles == "Medians:"
replace All = "" if Wealth_Quartiles == "Means:" | Wealth_Quartiles == "Medians:"
list

texsave Wealth_Quartiles First Second Third All using $folder\Final\table2raw.tex, frag title("Real nondurable spending before and after retirement.") footnote("*These values are not medians but percentiles, as indicated in the parentheses. \linebreak --- \linebreak This table references Table 2 of Hurd and Rohwedder's paper: Heterogeneity in spending change at retirement. \linebreak --- \linebreak Mean percent change is not reported because observation error on spending can produce large outliers when spending is put in ratio form. \linebreak --- \linebreak N = `count'.") hlines(1 4 5) replace
