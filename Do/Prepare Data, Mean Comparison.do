...
RAND (3) CAMS generated by ading up wave-consistent spending categoreis from 
individual waves by excluding house services, yard services, personal care,
...
preserve
	...
	//generate to check rand total 
	egen randtotal = rowtotal(h_cndur_real h_cdurs_real h_ctranss_real h_chouss_real)
	...
	replace datasetname = "Wave Consistent" if dataset == 3
	estpost tabstat total nondurables durables transportation housing, by(datasetname) nototal statistics(mean sd) columns(statistics) listwise
	esttab using $folder\Final\meanraw2.tex, main(mean) aux(sd) nostar noobs unstack label title("Mean Comparison between CAMS generated, RAND CAMS, and Wave Consistent CAMS generated spending categories (real adjusted)") note("\shortstack[l]{\\ The wave consistent measures are CAMS generated and exclude house services, \\ yard services, personal care, and household furnishings (which effect total and \\ nondurables). \\ --- \\ Most differences can be explained by the lack of imputation from the CAMS \\ generated values, which have been winsorized. \\ --- \\ The number of observations is 30124.}") replace
restore
